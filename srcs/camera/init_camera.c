...
void	init_camera(t_camera *cam, float aspect)
{
	...
	half_width = aspect * half_height;
	set_value_vector(&dist, cam->pos_x - cam->dir_x, cam->pos_y - cam->dir_y,
													cam->pos_z - cam->dir_z);
	dist = normalise(dist);
	u = normalise(prod_vector(new_vector(cam->up_x, cam->up_y, cam->up_z), dist));
	v = prod_vector(dist, u);
	cam->up_left.x = cam->pos_x - half_width * u.x + half_height * v.x - dist.x;
	cam->up_left.y = cam->pos_y - half_width * u.y + half_height * v.y - dist.y;
	cam->up_left.z = cam->pos_z - half_width * u.z + half_height * v.z - dist.z;
